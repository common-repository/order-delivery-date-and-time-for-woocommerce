...
class orddd_lite_process {
	...
	public static function orddd_lite_my_custom_checkout_field( $checkout = '' ) {
        ...
        if ( get_option( 'orddd_lite_enable_delivery_date' ) == 'on' ) {
            ...
            if( $is_delivery_enabled == 'yes' ) {
                ...
                $shippingconfig_dtotext_array = explode(',', get_option('orddd_lite_delivery_time_field_options'));
                $shippingconfig_dtovalue_array = explode(',', get_option('orddd_lite_delivery_time_field_values'));

                foreach ($shippingconfig_dtotext_array as $key => $value) {
                    $prefecture_name_fields[$shippingconfig_dtovalue_array[$key]] = $value;
                }
                ...
                if( '' == $checkout ) {
                    woocommerce_form_field( 'e_deliverydate', array(
                      ...
                      woocommerce_form_field('e_deliverydatetime', array(
                        'type' => 'select',
                        'label' => __(get_option('orddd_lite_delivery_time_field_label'), 'order-delivery-date-time'),
                        'options' => $prefecture_name_fields,
                        'required' => $validate_wpefield,
                        'placeholder' => __(get_option('orddd_lite_delivery_date_time_field_placeholder'), 'order-delivery-date-time'),
                        'custom_attributes' => array('style' => 'cursor:text !important;'),
                    ...
                    woocommerce_form_field('e_deliverydatetime', array(
                        ...
                        'custom_attributes' => array('style' => 'cursor:text !important;'),
                        'class' => array('form-row-wide')
                            ), $checkout->get_value('e_deliverydatetime'));
                }
            }
        }
    }
    ...
    public static function orddd_lite_my_custom_checkout_field_update_order_meta( $order_id ) {
        if ( isset( $_POST[ 'e_deliverydate' ] ) && $_POST[ 'e_deliverydate' ] != '' ) {
	      ...
	      update_post_meta($order_id, get_option('orddd_lite_delivery_time_field_label'), esc_attr($_POST['e_deliverydatetime']));
            ...
            if( $is_delivery_enabled == 'yes' ) {
                update_post_meta( $order_id, get_option( 'orddd_delivery_date_field_label' ), '' );
                 update_post_meta($order_id, get_option('orddd_lite_delivery_time_field_label'), '');
            }
        }
    }
    ...
    public static function orddd_lite_add_delivery_date_to_order_woo_new( $fields, $sent_to_admin, $order ) {
        ...
        $fields[ get_option( 'orddd_lite_delivery_date_field_label' ) ] = array(
           ...
           'label' => __(get_option('orddd_lite_delivery_time_field_label'), 'order-delivery-date-time'),
            'value' => get_post_meta($order_id, get_option('orddd_lite_delivery_time_field_label'), true),
    ...
    public static function orddd_lite_validate_date_wpefield() {
        ...
        if( isset( $_POST[ 'e_deliverydate' ] ) ) {
            $delivery_date = sanitize_text_field($_POST[ 'e_deliverydate' ]);
        } 
    ...
    public static function orddd_lite_add_delivery_date_to_order_page_woo( $order ) {
        ...
        $delivery_date_formatted = orddd_lite_common::orddd_lite_get_order_delivery_date( $order_id );
          $data = get_post_meta($order_id, get_option('orddd_lite_delivery_time_field_label'));
        ...
        if( $delivery_date_formatted != '' ) {
            ...
            echo '<p><strong>'.__( get_option( 'orddd_lite_delivery_time_field_label' ), 'order-delivery-date' ) . ':</strong> ' . $data[0] . '</p>';
